#include <EnvironmentVisualizer.hpp>
    ...
    EnvironmentVisualizer::EnvironmentVisualizer(rclcpp::Node::SharedPtr node,const std::shared_ptr<USVTransformHandler> usv_transform_handler, const std::shared_ptr<ObjectManager> object_manager): 
    node_(node), usv_transform_handler_(usv_transform_handler), object_manager_(object_manager) {
    ...
    for(unsigned int i = 0; i < object_manager_->get_objects().size(); i++){
        ...
        marker.pose.position.x = object_manager_->get_objects()[i]->get().position_x; // coordinate axis swapped
        marker.pose.position.y = object_manager_->get_objects()[i]->get().position_y;
...
void EnvironmentVisualizer::publish_usv_marker(){
    ...
    usv_marker.header.frame_id = "world_ned";
    ...
    usv_marker.color.a = 0.5;
    usv_marker.color.b = 1.0;
    usv_marker.pose.orientation = usv_transform_handler_->get_usv_position().rotation;
